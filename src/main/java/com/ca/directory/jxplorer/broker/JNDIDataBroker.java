...
//        schemaOps = new SchemaOps(ctx);
    ...
    public boolean hasConnectionError() {
        ...
        return ((ctx == null) || (connectionError));
    }
...
//XXX namingContexts may need to be explicitly asked for, since it is an op att.
            ...
            if (rootDNC == null || rootDNC.size() == 0)
                return new DN[]{new DN()}; // can't do anything; bail with an empty DN.
    ...
    public void SetContextToSearchingAliases() {
        ...
        try {
            if (ctx != null)
                ctx.addToEnvironment("java.naming.ldap.derefAliases", JXConfig.getProperty("option.ldap.searchAliasBehaviour"));
        } catch (Exception e) {
    ...
    public void SetContextToBrowsingAliases() {
        ...
        try {
            if (ctx != null)
                ctx.addToEnvironment("java.naming.ldap.derefAliases", JXConfig.getProperty("option.ldap.browseAliasBehaviour"));
        } catch (Exception e) {

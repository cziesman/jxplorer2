...
public class SearchModel {
    ...
    public SearchModel() {
        ...
        if (new File(searchFilterConfig).exists() == false) {
            log.info("no search filter config file found at: " + searchFilterConfig);
            return;
        }
        ...
        if (properties.size() == 0) {
            log.info("Initialising config file: " + searchFilterConfig);
            return;
        }
    }
    ...
    public ArrayList<String> getFilterNames(int type) {
        ...
        while (list.hasMoreElements()) {
            ...
            switch (type) {
                case BUILDFILTER: {
                    if (properties.get(temp).toString().startsWith("(") && temp.startsWith(NAME))
                        loadList.add(temp.substring(temp.indexOf(NAME) + 9));
                    break;
                }        //TE: get raw filter names.
                case ALLFILTERS: {
                    if (temp.startsWith(NAME)) loadList.add(temp.substring(temp.indexOf(NAME) + 9));
                    else if (temp.startsWith(TEXTNAME)) loadList.add(temp.substring(temp.indexOf(NAME) + 14));
                    break;
                }    //TE: get all filter names, not including prefix.
                case JOINFILTER: {
                    if (!properties.get(temp).toString().startsWith("(") && temp.startsWith(NAME))
                        loadList.add(temp.substring(temp.indexOf(NAME) + 9));
                    break;
                }    //TE: get joined filter names.
                case TEXTFILTER: {
                    if (temp.startsWith(TEXTNAME)) loadList.add(temp.substring(temp.indexOf(NAME) + 14));
                    break;
                }
                case FULLNAMES: {
                    if (temp.startsWith(NAME) || temp.startsWith(TEXTNAME)) loadList.add(temp);
                    break;
                }        //TE: get all filter names, including prefix.
                case BUILDJOIN: {
                    if (temp.startsWith(NAME)) loadList.add(temp.substring(temp.indexOf(NAME) + 9));
                    break;
                }        //TE: get all build and join filters.
                default:
                    loadList.add("");
            }
        }
...
//			name = name.substring(1);	//TE: ???? why did I need this???
     ...
     */
    protected void getDependantFilters(Object filterName) {                                                                    //TE: key=value.
